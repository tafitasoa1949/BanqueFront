 ...
 */
public class AddTanyCarteServlet extends HttpServlet {
    @EJB
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            try {
                ...
                int counteur = Integer.parseInt(request.getParameter("counteur"));
                if(counteur < 3){
                    String message = "Cliquer au moins 3 bornes";
                    request.setAttribute("errorMessage", message);
                    RequestDispatcher dispatcher = request.getRequestDispatcher("addTany.jsp");
                    ...
                    //coordonnes
                    Coordinate[] polygonCoordinates = new Coordinate[length];
                    for (int i = 0; i < length; i++) {
                        polygonCoordinates[i] = new Coordinate(Double.parseDouble(latitudes[i]), Double.parseDouble(longitudes[i]));
                    }
                    // Fermez la boucle en ajoutant le premier point comme dernier élément
                    polygonCoordinates[length] = polygonCoordinates[0];
                    for (Coordinate coordinate : polygonCoordinates) {
                        out.println("Latitude: " + coordinate.y + ", Longitude: " + coordinate.x);
                    }
                    // Créez une géométrie de type polygon à partir des coordonnées
                    /*GeometryFactory geometryFactory = new GeometryFactory();
                    LinearRing linearRing = geometryFactory.createLinearRing(polygonCoordinates);
                    Polygon polygon = geometryFactory.createPolygon(linearRing, null);
                    //perimetre
                    // Convertir le périmètre de degrés en mètres (supposons que vous travaillez avec des coordonnées géographiques)
                    double perimeterInDegrees = polygon.getLength();
                    // Conversion d'un degré de latitude/longitude en mètres (la valeur varie en fonction de la latitude)
                    double metersPerDegree = 111319.9; // Environ 111 km à l'équateur

                    double perimeterInMeters = perimeterInDegrees * metersPerDegree;
                    //surface
                    double areaDegrees = polygon.getArea();
                    double areaMeters = areaDegrees * (Math.pow(40008000 / 360, 2));
                    //Tany
                    Tany tany = new Tany();
                    tany.setCinproprietaire(cin);
                    tany.setSurface(areaMeters);
                    tany.setPerimetre(perimeterInMeters);
                    tany.setPrixunitaire(0);
                    tany.setTotalprix(0);
                    tany.setDate(dateAjourdhui);
                    tany.setColor("Red");
                    tany.setFillcolor("Blue");
                    //
